...
export function handleTokensDebt(event: TokensDebtEvent): void {
  ...
  if (idoPool !== null) {
    ...
    let userInfo = UserInfo.load(
      event.params.holder.toHex() + "-" + event.address.toHex()
    );
    if (userInfo == null) {
      ...
      userInfo.totalInvestedETH = userInfo.totalInvestedETH.plus(
        event.params.ethAmount
      );
      userInfo.save();
    }
    idoPool.totalInvestedETH = idoPool.totalInvestedETH.plus(
      event.params.ethAmount
    );
    idoPool.tokensForDistribution = idoPool.tokensForDistribution.plus(
...
export function handleTokensWithdrawn(event: TokensWithdrawnEvent): void {
  ...
  if (idoPool !== null) {
    ...
    idoPool.distributedTokens = idoPool.distributedTokens.plus(
      event.params.amount
    );
    idoPool.unsold = fetchUnsoldToken(event.address);
...
export function handleWithdrawNotSoldToken(
  event: WithdrawNotSoldTokenEvent
): void {
  let idoPool = IDOPool.load(event.address.toHex());
  if (idoPool !== null) {
